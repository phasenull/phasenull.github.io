...
interface Column {
  key: string;
  label: string;
  type: 'text' | 'number' | 'email' | 'url' | 'date' | 'select' | 'boolean';
  required?: boolean;
  editable?: boolean;
  sortable?: boolean;
  width?: string;
...
export default function DataTable({
  ...
  const handleSave = useCallback(async () => {
    ...
    const modifiedOrNewRows = data.filter(row => row._isModified || row._isNew);
    ...
    modifiedOrNewRows.forEach((row, index) => {
      columns.forEach(column => {
        ...
        if (column.type === 'email' && row[column.key] && !isValidEmail(row[column.key])) {
          validationErrors.push(`Row ${index + 1}: Invalid email format`);
        }
        
        if (column.type === 'url' && row[column.key] && !isValidURL(row[column.key])) {
  ...
  const handleSelectAll = useCallback((selected: boolean) => {
    ...
    setData(prevData => prevData.map(row => ({ ...row, _isSelected: selected })));
  }, [enableBulkActions]);
  ...
  const modifiedCount = data.filter(row => row._isModified || row._isNew).length;
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="text-sm text-gray-500">
            {data.length} rows {modifiedCount > 0 && `(${modifiedCount} modified)`}
          </div>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {enableDelete && selectedRows.length > 0 && (
            ...
            >
              {isLoading ? 'Deleting...' : `Delete Selected (${selectedRows.length})`}
            </button>
          )}
        </div>
      </div>
              ...
              {columns.map((column) => (
                <th
                  ...
                  className={`p-3 text-left border-b border-gray-200 font-medium text-gray-700 ${
                    column.sortable && enableSort ? 'cursor-pointer hover:bg-gray-100' : ''
                  }`}
            ...
            {sortedData.map((row, rowIndex) => (
              <tr
                ...
                className={`hover:bg-gray-50 ${
                  row._isNew ? 'bg-green-50' : row._isModified ? 'bg-yellow-50' : ''
...
function EditableCell({ value, column, onSave, disabled = false }: EditableCellProps) {
  ...
  if (disabled || !column.editable) {
    return <span className="text-gray-600">{formatDisplayValue(value, column)}</span>;
  }
...
// Helper Functions
function formatDisplayValue(value: any, column: Column): string {
